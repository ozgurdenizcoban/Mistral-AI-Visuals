...
async function callGemini({ prompt, maxTokens, temperature, jsonMode }) {
...
async function callMistral({ prompt, maxTokens, temperature, jsonMode }) {
  const key = Netlify.env.get("MISTRAL_API_KEY") || Netlify.env.get("VITE_MISTRAL_API_KEY");
...
export default async (request) => {
  ...
  try {
    ...
    try {
      const result = await callMistral(input);
